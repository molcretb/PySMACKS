...
import PySMACKS_GUI_PyQT5_registration
import PySMACKS_GUI_PyQT5_extraction
import PySMACKS_GUI_PyQT5_filtering
import PySMACKS_GUI_PyQT5_SEhist
import PySMACKS_GUI_PyQT5_kinetic
#from PySMACKS_GUI_PyQT5_registration import *
...
class Save_Window(QMainWindow):
    def __init__(self, registration_matrix, traces_data, traces_data_DO, traces_data_AO):
        ...
        # self.save_kinetic = QCheckBox("Kinetic rates")
        # self.save_kinetic.setToolTip('Save kinetic rates')
        # self.save_kinetic.setChecked(True)
        # layout1.addWidget(self.save_kinetic)
    ...
    def save_function(self, registration_matrix, traces_data, traces_data_DO, traces_data_AO):
        ...
        if fileName:
            ...
            # if self.save_kinetic.isChecked():
            #     self.save_data_JSON(kinetic_rates, fileName, 'kinetic_rates')
            
            # self.auto_save_calib['method'] = self.method_calib.currentText()
            # self.auto_save_calib['matrix_align'] = self.matrix_align
            
            # autosave_intermed_res(fileName, self.auto_save_calib)
    ...
    def save_data_JSON(self, traces_dict, file_save, type_data):
        ...
        # with open(name_complete, "w") as fp:
...
class metadata_Viewer(QMainWindow):
    def __init__(self, dataset):
        ...
        def display_traces_metadata(self, dataset_trace_i):
            ...
            #self.table.setItem(4, 0, QTableWidgetItem('trace_metadata'))
            #self.table.setItem(4, 1, QTableWidgetItem(str(dataset_trace_i.metadata)))
        ...
        # table.setColumnCount(2)
        # table.setHorizontalHeaderLabels(["Key", "Value"])
        # table.setRowCount(len(dataset.metadata))

        # for row, (key, value) in enumerate(dataset.metadata.items()):
        #     table.setItem(row, 0, QTableWidgetItem(str(key)))
        #     table.setItem(row, 1, QTableWidgetItem(str(value)))
        ...
        # table2.setHorizontalHeaderLabels(["Key", "Value"])
        # table2.setRowCount(len(dataset.sample_details))

        # for row, (key, value) in enumerate(dataset.sample_details.items()):
        #     table2.setItem(row, 0, QTableWidgetItem(str(key)))
        #     table2.setItem(row, 1, QTableWidgetItem(str(value)))
...
class FileSelectorApp(QMainWindow):
    def __init__(self):
        ...
        PySMACKS_GUI_PyQT5_registration.create_registration_tab(self)
        PySMACKS_GUI_PyQT5_extraction.create_extraction_tab(self)
        PySMACKS_GUI_PyQT5_filtering.create_filtering_tab(self)
        PySMACKS_GUI_PyQT5_SEhist.create_SEhist_tab(self)
        PySMACKS_GUI_PyQT5_kinetic.create_kinetic_tab(self)
        #self.create_FREThist_tab()
        #☺self.create_HMM_tab()
        ...
        #QShortcut(QKeySequence("2"), self, activated=lambda: self.label_choose.setCurrentIndex(1))
        #QShortcut(QKeySequence("3"), self, activated=lambda: self.label_choose.setCurrentIndex(2))
        QShortcut(QKeySequence(Qt.Key_Left), self, activated=lambda: PySMACKS_GUI_PyQT5_filtering.move_left(self))
        QShortcut(QKeySequence(Qt.Key_Right), self, activated=lambda: PySMACKS_GUI_PyQT5_filtering.move_right(self))
        QShortcut(QKeySequence("Return"), self, activated=lambda: PySMACKS_GUI_PyQT5_filtering.display_traces_plot(self))
        QShortcut(QKeySequence("Backspace"), self, activated=lambda: PySMACKS_GUI_PyQT5_filtering.clear_saved_traces(self))
        ...
        open_action.triggered.connect(lambda: PySMACKS_GUI_PyQT5_registration.open_file_dialog_acceptor(self))
        ...
        registQCAction.triggered.connect(lambda: PySMACKS_GUI_PyQT5_registration.regist_QC_plot(self))
    ...
    def load_traces_dataset(self):
        ...
        if file_name:
            ...
            if self.DD_trace_checkbox.isChecked():
                #self.line_DD, = self.ax_trace_plot.plot(np.array(self.traces_data[self.trace_IDs[0]].get('Intensity_DD')), 'orange')
            ...
            if self.DA_trace_checkbox.isChecked():
                #self.line_DA, = self.ax_trace_plot.plot(np.array(self.traces_data[self.trace_IDs[0]].get('Intensity_DA')),'red')
            ...
            if self.AA_trace_checkbox.isChecked():
                #self.line_AA, = self.ax_trace_plot.plot(np.array(self.traces_data[self.trace_IDs[0]].get('Intensity_AA')),'gray')
            ...
            if self.DD_DA_trace_checkbox.isChecked():
                #self.line_DD_DA, = self.ax_trace_plot.plot(np.array(self.traces_data[self.trace_IDs[0]].get('Intensity_DD')) + np.array(self.traces_data[self.trace_IDs[0]].get('Intensity_DA')),'blue')
    ...
    def drag_select(self, xmin, xmax):
        #global test_dict
        #print('OK')
        #self.ax[1].axvspan(xmin, xmax, color="red", alpha=0.3)
        ...
        #x = self.ax_trace_plot.lines[0].get_xdata()
        ...
        #     case "FRET":
        #         self.traces_data.traces[int(current_value)].metadata['indmin'] = int(self.indmin)
        #         self.traces_data.traces[int(current_value)].metadata['indmax'] = int(self.indmax)
                
        #     case "DO":
        #         self.traces_data.traces[int(current_value)].metadata['indmin_DO'] = int(self.indmin)
        #         self.traces_data.traces[int(current_value)].metadata['indmax_DO'] = int(self.indmax)
                
        #     case "AO":
        #         self.traces_data.traces[int(current_value)].metadata['indmin_AO'] = int(self.indmin)
        #         self.traces_data.traces[int(current_value)].metadata['indmax_AO'] = int(self.indmax)
        ...
        # self.selected_traces[current_value] = self.traces_data[current_value].copy()
        # self.selected_traces[current_value]['Intensity_DD'] = self.selected_traces[current_value]['Intensity_DD'][self.indmin:self.indmax]
        # self.selected_traces[current_value]['Intensity_DA'] = self.selected_traces[current_value]['Intensity_DA'][self.indmin:self.indmax]
        # self.selected_traces[current_value]['Intensity_AA'] = self.selected_traces[current_value]['Intensity_AA'][self.indmin:self.indmax]
        
        # self.selected_traces[current_value]['indmin'] = self.indmin
        # self.selected_traces[current_value]['indmax'] = self.indmax
    ...
    # def create_FREThist_tab(self):
    #     # Fourth tab
    #     tab4 = QWidget()
    #     layout4 = QVBoxLayout()
    #     label4 = QLabel("This is Tab 4")
    #     button4 = QPushButton("Button in Tab 4")
    #     layout4.addWidget(label4)
    #     layout4.addWidget(button4)
    #     tab4.setLayout(layout4)
        
    #     self.tabs.addTab(tab4, "FRET hist.")
    
    # def create_HMM_tab(self):
        
    #     # Fifth tab
    #     tab5 = QWidget()
    #     layout5 = QVBoxLayout()
    #     label5 = QLabel("This is Tab 5")
    #     button5 = QPushButton("Button in Tab 5")
    #     layout5.addWidget(label5)
    #     layout5.addWidget(button5)
    #     tab5.setLayout(layout5)

    #     # Add tabs to the QTabWidget
    #     self.tabs.addTab(tab5, "HMM analysis")
