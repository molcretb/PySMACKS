...
from PySMACKS.components.HMM_utils import *
import copy
from PySMACKS.GUI.GUI_SEhist import extract_labelled_traces
        ...
        for i in range(nb_hidden_states):
            for j in range(nb_hidden_states):
                if i != j:
                    ...
                    compt += 1
        ax_TbT_rates.boxplot(rate_dict.values(), tick_labels=list(rate_dict.keys()))
        ...
        for i in range(nb_hidden_states):
            for j in range(nb_hidden_states):
                if i != j:
                    ...
                    rate_dict[label_i] = stack_rate_mat[:,i,j]/framerate
                    compt += 1
        ax_EnsHMM_rates.boxplot(rate_dict.values(), tick_labels=list(rate_dict.keys()))
