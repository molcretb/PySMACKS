# -*- coding: utf-8 -*-
"""
Created on Wed Apr  1 13:51:48 2026
...
class TimeSeriesViewer(tk.Tk):
    ...
    def plot_series(self):
        ...
        if self.choiceDD.get() == 1:
            self.ax[0].plot(data_DD[:int(len(data_DD)*factor)], 'orange', label='DD')
            self.ax[1].plot(data_DD, 'orange', label='DD')
        if self.choiceDA.get() == 1:
            self.ax[0].plot(data_DA[:int(len(data_DA)*factor)], 'red', label='DA')
            self.ax[1].plot(data_DA, 'red', label='DA')
        if self.choiceAA.get() == 1:
            self.ax[0].plot(data_AA[:int(len(data_AA)*factor)], 'gray', label='AA')
            self.ax[1].plot(data_AA, 'gray', label='AA')
        if self.choiceDD_DA.get() == 1:
            self.ax[0].plot((np.array(data_DD[:int(len(data_DD)*factor)])+np.array(data_DA[:int(len(data_DA)*factor)])), 'blue', label='DD + DA')
            self.ax[1].plot(np.array(data_DD)+np.array(data_DA), 'blue', label='DD + DA')
        if self.choicebreak.get() == 1:
            self.ax[1].axvline(x = self.data_dict[series_name].get('bleaching_event_DD'), color = 'orange', label = 'Donor bleaching', ls='--');
            self.ax[1].axvline(x = self.data_dict[series_name].get('bleaching_event_AA'), color = 'gray', label = 'Acceptor bleaching', ls='--');
        ...
        if series_name in list(self.HMM_predict_dict.keys()):
            ...
            self.ax[0].plot(factor_HMM*HMM_seq[:int(len(HMM_seq)*factor)], 'green', label='HMM predict')
            self.ax[1].plot(factor_HMM*HMM_seq, 'green', label='HMM predict')
            
        self.ax[0].set_title(f"Time Series: {series_name}")
    ...
    def select_trace_button(self):
        current_value = self.series_var.get()
        self.data_dict_selected[current_value] = self.data_dict[current_value].copy()
        self.choice_trace.set(1)
    ...
    def onselect(self, xmin, xmax):
        ...
        current_value = self.series_var.get()
        self.data_dict_selected[current_value] = self.data_dict[current_value].copy()
        self.data_dict_selected[current_value]['Intensity_DD'] = self.data_dict_selected[current_value]['Intensity_DD'][indmin:indmax]
        self.data_dict_selected[current_value]['Intensity_DA'] = self.data_dict_selected[current_value]['Intensity_DA'][indmin:indmax]
        self.data_dict_selected[current_value]['Intensity_AA'] = self.data_dict_selected[current_value]['Intensity_AA'][indmin:indmax]
        self.choice_trace.set(1)
        #onselect.xmin = indmin
        #onselect.xmax = indmax
    ...
    def on_scroll(self, event):
        ...
        self.ax[1].set_xlim([event.xdata - new_width * (1 - relx),
                          event.xdata + new_width * relx])
        self.ax[1].set_ylim([event.ydata - new_height * (1 - rely),
