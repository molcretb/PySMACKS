...
class QCRegistrationWindow(QMainWindow):
    ...
    def QC_regis_plot(self, img_stack_acceptor, img_stack_donor, matrix_align, method, radius):
        # img_stack_acceptor, img_stack_donor, V = 0, U = 0, matrix_align = 0, k = 0, kernel_size = 2, min_distance = 1, method = 'OpticalFLow'):
...
def create_registration_tab(self):
    ...
    button_acceptor_calib.clicked.connect(lambda: open_file_dialog_acceptor(self))
    ...
    button_donor_calib.clicked.connect(lambda: open_file_dialog_donor(self))
...
def show_loaded_matrix(self):
    ...
    img_stack_donor = np.zeros((img_donor.height, img_donor.width, 10))
    img_stack_acceptor = np.zeros((img_acceptor.height, img_acceptor.width, 10))
        
    img_donor.seek(0)
    img_acceptor.seek(0)
    img_stack_donor[:,:,0] = np.array(img_donor)
    img_stack_acceptor[:,:,0] = np.array(img_acceptor)
    
    method = self.method_calib.currentText()

    if method == 'Optical Flow':
        
        img_warp_donor = Warp_OpticalFlow(img_stack_donor, self.matrix_align[0], self.matrix_align[1])
...
def update_contrast(self):
    # Read slider values and update contrast factors
    # for channel in ['Acceptor', 'Donor']:
    #     value = self.sliders[channel].value()
    #     self.contrast_factors[channel] = value / 100.0

    # Apply contrast adjustments
    
    #♣ self.figure.clear()
    #self.img_calib.set_data()
    ...
    #R_max = self.R_channel >= np.max(self.R_channel) * self.sliders['Acceptor'].value()
    #G_max = self.G_channel >= np.max(self.G_channel) * self.sliders['Donor'].value()
    ...
    #self.img_calib.set_data(np.stack((self.R_channel, self.G_channel, self.B_channel), axis=-1))
...
def open_file_dialog_acceptor(self):
...
def open_file_dialog_donor(self):
    # Open file dialog and get selected file path
    options = QFileDialog.Options()
    # You can set options like QFileDialog.DontUseNativeDialog if needed
